...
import com.rsl.youresto.databinding.ActivityKitchenPrint50Binding
...
class KitchenPrint50Activity : AppCompatActivity() {

    private lateinit var mBinding: ActivityKitchenPrint50Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_kitchen_print50)
        ...
        mLocationType = mSharedPrefs.getInt(LOCATION_SERVICE_TYPE, 0)
...
//        super.onResume()
//        this.window!!.setLayout(500, ViewGroup.LayoutParams.WRAP_CONTENT)
...
//    }
    ...
    private fun init() {
        mCartID = intent.getStringExtra("CART_ID")
        if (mLocationType == SERVICE_DINE_IN) {
            mTableID = mSharedPrefs.getString(SELECTED_TABLE_ID, "")!!
            mTableNO = mSharedPrefs.getInt(SELECTED_TABLE_NO, 0)
        } else if (mLocationType == SERVICE_QUICK_SERVICE) {
    ...
    private fun setDineInPrintLayout() {
        mBinding.constraintQuickOptions.visibility = GONE
...
//            e(javaClass.simpleName, "ProductList size: ${mSingleKOTData!!.mProductList.size}")
//
//        }
    ...
    private fun printKitchen() {
        ...
        when (mSingleKOTData!!.mKitchenModel.mPrinterType) {
            BLUETOOTH_PRINTER -> mExecutor!!.diskIO().execute {
                ...
                when {
                    mPrinterName.equals("InnerPrinter", ignoreCase = true) -> {
                        ...
                        try {
                            e("TAG", "splitImage2: height: $height" )
            ...
            NETWORK_PRINTER -> mExecutor!!.networkIO().execute {
                try {
                    ...
                    when {
                        mHeight < 200 -> mBluetoothUtil.splitImage2(
                            ...
                            mHeight = height / mRows.roundToInt()
                            ...
                            when {
                                mHeight < 200 -> mBluetoothUtil.splitImage2(
                                    ...
                                    when {
                                        mHeight < 200 -> mBluetoothUtil.splitImage2(
                                            bitmap,
                                            mRows.roundToInt(), 1, NETWORK_PRINTER, mSingleKOTData!!.mKitchenModel, 1
                                        )
                                    }
                                }
                            }
                    ...
                    e.printStackTrace()
                    CustomToast.makeText(this, "Printing Failed Exception: ${e.localizedMessage}", Toast.LENGTH_SHORT)
                        .show()
                    finish()
                }
    ...
    fun onGetDataFromCart(mEvent: KitchenBundleEvent) {
        if (mEvent.mResult) {
            ...
            e(
                ...
                "onGetDataFromCart: mSerialNO: $mSerialNO , mGroupName: $mGroupName , mCartID: $mCartID, " +
