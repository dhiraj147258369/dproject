package com.rsl.foodnairesto.data.database_download.network
...
import com.rsl.foodnairesto.utils.AppConstants.API_DATE
...
import com.rsl.foodnairesto.utils.AppConstants.QUICK_SERVICE_TABLE_ID
import com.rsl.foodnairesto.utils.AppConstants.QUICK_SERVICE_TABLE_NO
...
import com.rsl.foodnairesto.utils.EndPoints.API_IP
import com.rsl.foodnairesto.utils.EndPoints.CART_REPORT
import com.rsl.foodnairesto.utils.EndPoints.FAVORITE_ITEMS
import com.rsl.foodnairesto.utils.EndPoints.GET_ALL_DATA
...
object DownloadDatabaseNetworkUtils {
    ...
    fun getServerData(mServerArray: JSONArray): ArrayList<ServerModel> {
        ...
        for (i in 0 until mServerArray.length()) {
            ...
            val mLocationsUser=ArrayList<LocationUserList>()
            var userLocationArray=mSingleUser.getJSONArray("location")
            for(j in 0 until userLocationArray.length()){
                var addBoolean=true
                var mSingleLocation=userLocationArray.getJSONObject(j)
                for(k in 0 until mLocationsUser.size){
                    if(mSingleLocation.getString("location_id").equals(mLocationsUser[k].mLocation_Id)){
                        addBoolean=false
                    }
                }
                if(addBoolean) {
                    mLocationsUser.add(
                        LocationUserList(
                            ...
                            mSingleLocation.getString("location_type")
                        )
                    )
                }
            }
            ...
            val mUserModel = ServerModel(
                ...
                mSingleUser.getString("usertype"),
                mLocationsUser,
                mUserPermissionList
            )
        ...
        for (i in 0 until mGroupArray.length()) {
            ...
            for (j in 0 until mCategoryArray.length()) {
                ...
                for ((mProductSequence, k) in (0 until mProductArray.length()).withIndex()) {
                    ...
                    val mPriceArray= mSingleProduct.getJSONArray("tablewise_price")
                    ...
                    val priceArrayModel= ArrayList<TableWisePrice>()
                    for(l in 0 until mPriceArray.length()){
                        val mSinglePrice=mPriceArray.getJSONObject(l)
                        priceArrayModel.add(TableWisePrice(mSinglePrice.getString("table_category_id"),mSinglePrice.getLong("price").toBigDecimal()))
                    }
