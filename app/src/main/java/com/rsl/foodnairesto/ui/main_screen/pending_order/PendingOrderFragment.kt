...
import com.rsl.foodnairesto.databinding.FragmentPendingOrderBinding
...
import com.rsl.foodnairesto.utils.Animations.runLayoutAnimationFallDown
...
//                    mBinding.spinnerServer.setSelection(mSelectedServerPosition)
    ...
    private fun setUpRecyclerView(mServerID: String) {
        ...
        if(mServerID.length==0) {
            ...
            cartViewModel.getPendingOrderCartDatafilter(mServerID).observe(viewLifecycleOwner) {
                if (it.isNotEmpty()) {
                    mCartList = ArrayList(it)
