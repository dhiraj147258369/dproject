...
import com.rsl.foodnairesto.databinding.FragmentAddToTabBinding
...
import com.rsl.foodnairesto.ui.main_screen.tables_and_tabs.add_to_tab.tabs.modifier_2.ModifierType2Event2

import com.rsl.foodnairesto.ui.main_screen.tables_and_tabs.tables.TablesFragment
...
import com.rsl.foodnairesto.utils.AppConstants.LOGGED_IN_SERVER_NAME
import com.rsl.foodnairesto.utils.AppConstants.SELECTED_LOCATION_ID
import com.rsl.foodnairesto.utils.AppConstants.SELECTED_TABLE_ID
import com.rsl.foodnairesto.utils.AppConstants.SELECTED_TABLE_NO
...
class AddToTabFragment : Fragment() {
    ...
    private var pricew:BigDecimal= BigDecimal(0)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Log.e("table_get",mTableID)
        mTableNO = mSharedPrefs.getInt(SELECTED_TABLE_NO, 0)
        Log.e("tableno_get",mTableID)
        setupProduct()
    }
    ...
    private fun setupProduct() {
        ...
        productViewModel.getProduct(mProductID).observe(viewLifecycleOwner) {
            it?.let {
                ...
                if(mProductModel.mTablewisePrice.size>0) {
                    for (price in mProductModel.mTablewisePrice) {
                        if (mTableID == price.mTableid) {
                            val mProductPrice: String = "${requireActivity().resources.getString(R.string.string_currency_sign)} " + String.format(
                                Locale.ENGLISH,
                                "%.2f",
                                price.mPricew
                            )
                            pricew=price.mPricew
                            ...
                            pricew=mProductModel.mDineInPrice
                            mBinding.textViewProductPrice.text = mProductPrice
                        }
                    }
                }else{
...
//                mBinding.textViewProductPrice.text = mProductPrice
    ...
    private fun addProduct() {
        ...
        val mProductUnitPrice = when (mSelectedLocationType) {
            SERVICE_DINE_IN -> pricew
            SERVICE_QUICK_SERVICE -> pricew
    ...
    private var commentUser:String=""

    @Subscribe
    fun getInstructions(mEvent: ModifierType2Event2){
        mSpecialInstruction=mEvent.commentUser
    }
    ...
    fun onProductPriceUpdate(mEvent: ModifierType2Event) {
        ...
        when (mEvent.mProductType) {
            2 ->
            {
                ...
                mTotalProductPrice = (pricew + ingredientPrice) * mQuantity
